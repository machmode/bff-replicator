...
+ Added rayon pthread parallelisation option (use --parallel in the command line). Gives a
  speedup of ~1.7x, noticeeable on large population sizes

### v1-03 260308
+ Addeed support to show most common tape AND most common replicator (with instruction code),
  togeter with hex byes, since it is plausible that the most common tape has no instruction code
...
+ Set seed at start if not passed as a commnd line arg so that it is user visible for recreation of 
  simulation runs
